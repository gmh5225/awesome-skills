...
| [agentic-reserve/blockint-skills](https://github.com/agentic-reserve/blockint-skills) | Public Cursor skill pack for blockchain intelligence (**45 directories**): on-chain research, clustering/attribution, compliance-screening concepts, DeFi/MEV and Solana investigation playbooks, Dune/OSINT/Katana routing, and security-research references |
...
| [yaklang/hack-skills](https://github.com/yaklang/hack-skills) | **HACK.SKILLS**: offensive-security knowledge base for agents (master + category routers + deep-topic skills) across web/API/auth, OS/AD, mobile, pwn/reverse/crypto, blockchain, AI/LLM security, network pivoting, and forensics; installable via `npx skills add yaklang/hack-skills` |
| [ljagiello/ctf-skills](https://github.com/ljagiello/ctf-skills) | **11 CTF skills**: web exploitation, binary pwn, crypto, reverse engineering, forensics, OSINT, malware, misc, AI/ML; orchestrator (`solve-challenge`) + writeup; installable via `npx skills add ljagiello/ctf-skills` |
...
| [cat-plan](https://github.com/fengzizz/associate-cat/tree/main/skills/cat-plan) | An easy-to-use AI planning skill that combines in-depth analysis with fast convergence on a clear plan—even for large, complex codebases. |
...
| [ghidra-re-skill](https://github.com/OwenPawl/ghidra-re-skill) | Codex-focused Ghidra reverse engineering skill with headless workflows, bug-hunting bundles, and a live bridge for Apple Mach-O analysis |
| [ghidra-rpc](https://github.com/cellebrite-labs/ghidra-rpc) | Agentic Ghidra RE skill (Cellebrite Labs): runs Ghidra as a persistent daemon exposing a JSON CLI — decompile, trace call graphs, rename/annotate, recover types, patch, and diff binaries; drivable from any shell-capable agent (pi/Claude Code/Cursor) |
| [Claude-Red](https://github.com/SnailSploit/Claude-Red) | Curated offensive-security skill library for Claude: structured `SKILL.md` playbooks across attack surfaces (e.g., SQLi, shellcode, EDR evasion, exploit development) |
| [bug-hunter](https://github.com/codexstar69/bug-hunter) | Adversarial AI bug-hunter + auto-fix skill for multi-agent coding CLIs: finds security vulnerabilities, logic/runtime bugs, and applies autonomous fixes on a safe branch |
| [Android-Pentesting-Skill](https://github.com/DragonJAR/Android-Pentesting-Skill) | World-class Android pentesting skill for AI agents: static analysis, dynamic instrumentation, RASP detection, authorized bypass validation, source-to-sink tracing, MASVS scoring, and professional reporting in one DRY workflow |
| [pentest_skill](https://github.com/wudidike/pentest_skill) | Black-box web penetration testing automation framework for AI agents with phase-based pipeline (fingerprinting → recon → vulns → evidence/report), unified CLI, and tool-fallback strategy for missing external binaries |
...
- [Open Agent Relay](https://github.com/ShakespeareLabs/open-agent-relay) - Local-first CLI that lets teammates and agents call bounded Claude Code, Codex, or automation capabilities over a trusted LAN while the publisher keeps prompts, credentials, dependencies, and working directories local.
